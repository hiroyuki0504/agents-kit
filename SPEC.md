...
8. **ブートストラップ push**（.agents 一式を main に載せる。導入デッドロックの解消。`--no-push` 時はスキップして内容確認を促し、「push は install.sh 再実行で行え」と表示）:
   ...
   **8.5 導入クローンの checkout 追いつき (impl-fix)**: 手順 8 の内容が remote main に載った後（push 成功 or 冪等スキップ時のみ。--no-push / dirty スキップ時は行わない）、次の条件を**全て**満たす場合に限り、導入クローンの checkout を FF で TIP に追いつかせる: (i) checkout 中のブランチが `<main>`、(ii) `merge-base --is-ancestor HEAD TIP`（分岐していない）、(iii) 追跡ファイルに未コミット変更なし（`status --porcelain -uno` 空）。FF を阻む未追跡ファイルは「TIP の同名 blob と内容同一」のものだけを削除して FF する（FF 後に追跡状態で復活するため無損失）。内容が異なる未追跡ファイルに当たったら削除分を blob から復元して中止し、1 行の案内のみ出す。理由: これをしないと導入直後の `git pull` が「未追跡の .agents 等に上書きされる」で必ず拒否される（実測）。条件不成立時は案内のみでユーザーの状態に触れない。
